...
class Attribute(expr):
    ...
    def _to_builtin(self):
        return ast.Attribute(
            attr=wast_to_node(self.attr),
            value=wast_to_node(self.value),
            ctx=wast_to_node(self.ctx),
        )

    @classmethod
    def _from_builtin(cls, node):
        return cls(
            attr=node_to_wast(node.attr),
            value=node_to_wast(node.value),
            ctx=node_to_wast(node.ctx),
        )
...
class List(expr):
    ...
    def _to_builtin(self):
        return ast.List(ctx=wast_to_node(self.ctx), elts=wast_to_node(self.elts))

    @classmethod
    def _from_builtin(cls, node):
        return cls(ctx=node_to_wast(node.ctx), elts=node_to_wast(node.elts))
...
class Tuple(expr):
    ...
    def _to_builtin(self):
        return ast.Tuple(ctx=wast_to_node(self.ctx), elts=wast_to_node(self.elts))
